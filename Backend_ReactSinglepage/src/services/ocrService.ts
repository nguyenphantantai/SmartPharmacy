...
export function extractPrescriptionInfo(ocrText: string): ExtractedPrescriptionInfo {
  ...
  const hospitalPatterns = [
    ...
    // Improved: Capture full name including parentheses and multiple words
    /Tên\s*đơn\s*vi[ị]?[:\s]+([A-ZÀÁẠẢÃÂẦẤẬẨẪĂẰẮẶẲẴÈÉẸẺẼÊỀẾỆỂỄÌÍỊỈĨÒÓỌỎÕÔỒỐỘỔỖƠỜỚỢỞỠÙÚỤỦŨƯỪỨỰỬỮỲÝỴỶỸĐ][A-ZÀÁẠẢÃÂẦẤẬẨẪĂẰẮẶẲẴÈÉẸẺẼÊỀẾỆỂỄÌÍỊỈĨÒÓỌỎÕÔỒỐỘỔỖƠỜỚỢỞỠÙÚỤỦŨƯỪỨỰỬỮỲÝỴỶỸĐ\s()]+?)(?:\s+\d+\s+[ẦẤẬẨẪĂẰẮẶẲẴÈÉẸẺẼÊỀẾỆỂỄÌÍỊỈĨÒÓỌỎÕÔỒỐỘỔỖƠỜỚỢỞỠÙÚỤỦŨƯỪỨỰỬỮỲÝỴỶỸĐ]|\s|$|Địa|Dia|Số|So|Phone|ĐT|PK|Phòng|Mã|Ma|toa|mp)/i,
  ...
  const diagnosisPatterns = [
    ...
    // Pattern 5: "Chẩn đoán" with ICD code pattern (H00, K21, I10, etc.) - more flexible, allow semicolon
    /(?:Chẩn\s*đoán|Chan\s*doan|Chân\s*đoán)[:\s]*([A-Z]\d{2,3}(?:\s*[-–]\s*)?[A-ZÀÁẠẢÃÂẦẤẬẨẪĂẰẮẶẲẴÈÉẸẺẼÊỀẾỆỂỄÌÍỊỈĨÒÓỌỎÕÔỒỐỘỔỖƠỜỚỢỞỠÙÚỤỦŨƯỪỨỰỬỮỲÝỴỶỸĐa-zàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ\s\-–;]+?)(?:\s*(?:Cận|Can|Mạch|Mach|Huyết|Huyet|Thân|Than|Ghi|Lời|Loi|Ngày|Ngay|BS|Bác|Bac|\d+\s*\)\s*[A-Z]|SIMETHICON|MALTAGIT|PARACETAMOL|CALCI|VITAMIN|TOBRAMYCIN|AGICLARI|MEDSOLU|KACERIN|PANACTOL|Paracetamol|Acepron|AGI-CALCI)|$)/i,
