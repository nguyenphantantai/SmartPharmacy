...
export default function FaceMaskPage() {
  ...
  const faceMaskProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Mặt nạ dưỡng da");
  }, []);

  const brands = useMemo(() => {
    const allBrands = faceMaskProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [faceMaskProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Mặt nạ dưỡng da</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Mặt nạ dưỡng da</h1>
