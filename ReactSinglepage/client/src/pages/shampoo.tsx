...
export default function ShampooPage() {
  ...
  const shampooProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Dầu gội đầu");
  }, []);

  const brands = useMemo(() => {
    const allBrands = shampooProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [shampooProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Dầu gội đầu</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Dầu gội đầu</h1>
