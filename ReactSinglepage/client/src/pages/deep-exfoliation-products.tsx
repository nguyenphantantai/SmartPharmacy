...
export default function DeepExfoliationProductsPage() {
  ...
  const deepExfoliationProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Tẩy tế bào chết chuyên sâu");
  }, []);

  const brands = useMemo(() => {
    const allBrands = deepExfoliationProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [deepExfoliationProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = deepExfoliationProducts;
    ...
    return filtered;
  }, [deepExfoliationProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Tẩy tế bào chết chuyên sâu</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Tẩy tế bào chết chuyên sâu</h1>
