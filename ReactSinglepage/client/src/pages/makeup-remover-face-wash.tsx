...
export default function MakeupRemoverFaceWashPage() {
  ...
  const makeupRemoverFaceWashProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Nước tẩy trang & sữa rửa mặt");
  }, []);

  const brands = useMemo(() => {
    const allBrands = makeupRemoverFaceWashProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [makeupRemoverFaceWashProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = makeupRemoverFaceWashProducts;
    ...
    return filtered;
  }, [makeupRemoverFaceWashProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Nước tẩy trang & sữa rửa mặt</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Nước tẩy trang & sữa rửa mặt</h1>
