...
export default function MouthwashPage() {
  ...
  const mouthwashProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Nước súc miệng");
  }, []);

  const brands = useMemo(() => {
    const allBrands = mouthwashProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = mouthwashProducts;
    ...
    return filtered;
  }, [mouthwashProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Nước súc miệng</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Nước súc miệng</h1>
