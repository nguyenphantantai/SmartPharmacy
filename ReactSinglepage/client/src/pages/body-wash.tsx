...
export default function BodyWashPage() {
  ...
  // Lọc sản phẩm sữa tắm
  const bodyWashProducts = useMemo(() => {
    return personalCareProducts.filter(product => 
      product.category === "Sản phẩm phòng tắm" && product.subCategory === "Sữa tắm"
    );
  }, []);

  const brands = useMemo(() => {
    const allBrands = bodyWashProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [bodyWashProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sữa tắm</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sữa tắm</h1>
