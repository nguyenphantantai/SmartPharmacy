...
export default function AcneSupportPage() {
  ...
  const acneSupportProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Hỗ trợ giảm mụn");
  }, []);

  const brands = useMemo(() => {
    const allBrands = acneSupportProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = acneSupportProducts;
    ...
    return filtered;
  }, [acneSupportProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Hỗ trợ giảm mụn</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Hỗ trợ giảm mụn</h1>
