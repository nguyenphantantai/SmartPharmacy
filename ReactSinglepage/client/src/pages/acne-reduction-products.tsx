...
export default function AcneReductionProductsPage() {
  ...
  const acneReductionProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Sản phẩm hỗ trợ giảm mụn");
  }, []);

  const brands = useMemo(() => {
    const allBrands = acneReductionProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [acneReductionProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = acneReductionProducts;
    ...
    return filtered;
  }, [acneReductionProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sản phẩm hỗ trợ giảm mụn</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sản phẩm hỗ trợ giảm mụn</h1>
