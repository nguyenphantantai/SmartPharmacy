...
export default function ConditionerPage() {
  ...
  const conditionerProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Dầu xả");
  }, []);

  const brands = useMemo(() => {
    const allBrands = conditionerProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = conditionerProducts;
    ...
    return filtered;
  }, [conditionerProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Dầu xả</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Dầu xả</h1>
