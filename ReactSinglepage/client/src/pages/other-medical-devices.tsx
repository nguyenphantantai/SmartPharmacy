...
export default function OtherMedicalDevicesPage() {
  ...
  const otherMedicalDevicesProducts = useMemo(() => {
    return medicalDevicesProducts.filter(product => product.category === "Thiết bị y tế khác");
  }, []);

  const brands = useMemo(() => {
    const allBrands = otherMedicalDevicesProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [otherMedicalDevicesProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = otherMedicalDevicesProducts;
    ...
    return filtered;
  }, [otherMedicalDevicesProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Thiết bị y tế khác</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Thiết bị y tế khác</h1>
