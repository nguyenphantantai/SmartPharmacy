...
export default function BloodPressureMonitorsPage() {
  ...
  const bloodPressureMonitorsProducts = useMemo(() => {
    return medicalDevicesProducts.filter(product => product.category === "Máy đo huyết áp");
  }, []);

  const brands = useMemo(() => {
    const allBrands = bloodPressureMonitorsProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [bloodPressureMonitorsProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = bloodPressureMonitorsProducts;
    ...
    return filtered;
  }, [bloodPressureMonitorsProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Máy đo huyết áp</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Máy đo huyết áp</h1>
