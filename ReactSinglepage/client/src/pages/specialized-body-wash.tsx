...
export default function SpecializedBodyWashPage() {
  ...
  const specializedBodyWashProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Sữa tắm chuyên sâu");
  }, []);

  const brands = useMemo(() => {
    const allBrands = specializedBodyWashProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [specializedBodyWashProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = specializedBodyWashProducts;
    ...
    return filtered;
  }, [specializedBodyWashProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sữa tắm chuyên sâu</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sữa tắm chuyên sâu</h1>
