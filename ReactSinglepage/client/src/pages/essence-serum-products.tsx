...
export default function EssenceSerumProductsPage() {
  ...
  const essenceSerumProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Tinh chất & Serum");
  }, []);

  const brands = useMemo(() => {
    const allBrands = essenceSerumProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [essenceSerumProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = essenceSerumProducts;
    ...
    return filtered;
  }, [essenceSerumProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Tinh chất & Serum</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Tinh chất & Serum</h1>
