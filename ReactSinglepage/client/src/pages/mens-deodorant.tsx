...
export default function MensDeodorantPage() {
  ...
  const mensDeodorantProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Khử mùi cho nam");
  }, []);

  const brands = useMemo(() => {
    const allBrands = mensDeodorantProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [mensDeodorantProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = mensDeodorantProducts;
    ...
    return filtered;
  }, [mensDeodorantProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Khử mùi cho nam</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Khử mùi cho nam</h1>
