...
export default function SprayDeodorantPage() {
  ...
  // Lọc sản phẩm xịt khử mùi
  const sprayProducts = useMemo(() => {
    return personalCareProducts.filter(product => 
      product.category === "Sản phẩm khử mùi" && product.subCategory === "Xịt khử mùi"
    );
  }, []);

  const brands = useMemo(() => {
    const allBrands = sprayProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [sprayProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Xịt khử mùi</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Xịt khử mùi</h1>
