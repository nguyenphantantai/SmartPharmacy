...
export default function MensHairCarePage() {
  ...
  const mensHairCareProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Chăm sóc tóc cho nam");
  }, []);

  const brands = useMemo(() => {
    const allBrands = mensHairCareProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [mensHairCareProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = mensHairCareProducts;
    ...
    return filtered;
  }, [mensHairCareProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Chăm sóc tóc cho nam</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Chăm sóc tóc cho nam</h1>
