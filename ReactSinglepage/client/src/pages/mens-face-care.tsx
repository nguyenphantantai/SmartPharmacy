...
export default function MensFaceCarePage() {
  ...
  const mensFaceCareProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Chăm sóc da mặt cho nam");
  }, []);

  const brands = useMemo(() => {
    const allBrands = mensFaceCareProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [mensFaceCareProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = mensFaceCareProducts;
    ...
    return filtered;
  }, [mensFaceCareProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Chăm sóc da mặt cho nam</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Chăm sóc da mặt cho nam</h1>
