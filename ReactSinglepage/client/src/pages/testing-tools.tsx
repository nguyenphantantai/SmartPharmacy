...
export default function TestingToolsPage() {
  ...
  const testingToolsProducts = useMemo(() => {
    return medicalDevicesProducts.filter(product => product.category === "Dụng cụ kiểm tra");
  }, []);

  const brands = useMemo(() => {
    const allBrands = testingToolsProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [testingToolsProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = testingToolsProducts;
    ...
    return filtered;
  }, [testingToolsProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Dụng cụ kiểm tra</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Dụng cụ kiểm tra</h1>
