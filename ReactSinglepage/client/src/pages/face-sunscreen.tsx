...
export default function FaceSunscreenPage() {
  ...
  const faceSunscreenProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Kem chống nắng dành cho mặt");
  }, []);

  const brands = useMemo(() => {
    const allBrands = faceSunscreenProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [faceSunscreenProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = faceSunscreenProducts;
    ...
    return filtered;
  }, [faceSunscreenProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Kem chống nắng dành cho mặt</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Kem chống nắng dành cho mặt</h1>
