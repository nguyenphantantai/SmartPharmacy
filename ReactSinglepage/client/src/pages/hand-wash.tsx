...
export default function HandWashPage() {
  ...
  // Lọc sản phẩm nước rửa tay
  const handWashProducts = useMemo(() => {
    return personalCareProducts.filter(product => 
      product.category === "Sản phẩm phòng tắm" && product.subCategory === "Nước rửa tay"
    );
  }, []);

  const brands = useMemo(() => {
    const allBrands = handWashProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [handWashProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Nước rửa tay</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Nước rửa tay</h1>
