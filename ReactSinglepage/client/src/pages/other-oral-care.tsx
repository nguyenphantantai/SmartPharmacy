...
export default function OtherOralCarePage() {
  ...
  const otherOralCareProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Sản phẩm chăm sóc răng miệng khác");
  }, []);

  const brands = useMemo(() => {
    const allBrands = otherOralCareProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [otherOralCareProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = otherOralCareProducts;
    ...
    return filtered;
  }, [otherOralCareProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sản phẩm chăm sóc răng miệng khác</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sản phẩm chăm sóc răng miệng khác</h1>
