...
export default function ThermometersPage() {
  ...
  const thermometersProducts = useMemo(() => {
    return medicalDevicesProducts.filter(product => product.category === "Nhiệt kế");
  }, []);

  const brands = useMemo(() => {
    const allBrands = thermometersProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [thermometersProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = thermometersProducts;
    ...
    return filtered;
  }, [thermometersProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Nhiệt kế</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Nhiệt kế</h1>
