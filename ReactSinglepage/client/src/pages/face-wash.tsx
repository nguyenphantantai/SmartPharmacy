...
export default function FaceWashPage() {
  ...
  const faceWashProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Sữa rửa mặt");
  }, []);

  const brands = useMemo(() => {
    const allBrands = faceWashProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [faceWashProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sữa rửa mặt</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sữa rửa mặt</h1>
