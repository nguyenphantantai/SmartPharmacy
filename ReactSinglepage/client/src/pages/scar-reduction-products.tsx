...
export default function ScarReductionProductsPage() {
  ...
  const scarReductionProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Sản phẩm hỗ trợ giảm sẹo");
  }, []);

  const brands = useMemo(() => {
    const allBrands = scarReductionProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [scarReductionProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = scarReductionProducts;
    ...
    return filtered;
  }, [scarReductionProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sản phẩm hỗ trợ giảm sẹo</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sản phẩm hỗ trợ giảm sẹo</h1>
