...
export default function DentalFlossPage() {
  ...
  const dentalFlossProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Tăm chỉ nha khoa");
  }, []);

  const brands = useMemo(() => {
    const allBrands = dentalFlossProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = dentalFlossProducts;
    ...
    return filtered;
  }, [dentalFlossProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Tăm chỉ nha khoa</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Tăm chỉ nha khoa</h1>
