...
export default function FaceExfoliationPage() {
  ...
  const faceExfoliationProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Tẩy tế bào chết cho mặt");
  }, []);

  const brands = useMemo(() => {
    const allBrands = faceExfoliationProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [faceExfoliationProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = faceExfoliationProducts;
    ...
    return filtered;
  }, [faceExfoliationProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink href="/cham-soc-mat">Chăm sóc mặt</BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Tẩy tế bào chết cho mặt</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Tẩy tế bào chết cho mặt</h1>
