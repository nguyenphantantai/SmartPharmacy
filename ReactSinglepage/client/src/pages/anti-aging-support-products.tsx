...
export default function AntiAgingSupportProductsPage() {
  ...
  const antiAgingSupportProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Hỗ trợ giảm lão hóa");
  }, []);

  const brands = useMemo(() => {
    const allBrands = antiAgingSupportProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [antiAgingSupportProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = antiAgingSupportProducts;
    ...
    return filtered;
  }, [antiAgingSupportProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Hỗ trợ giảm lão hóa</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Hỗ trợ giảm lão hóa</h1>
