...
import BodyCareProductGrid from "../components/body-care-product-grid";
import { useState, useMemo } from "react";
import { personalCareProducts } from "@/data/personal-care";
import { Link } from "wouter";

export default function BodyCareProductsPage() {
  ...
  // Lọc sản phẩm chăm sóc cơ thể
  const bodyCareProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.category === "Chăm sóc cơ thể");
  }, []);

  const brands = useMemo(() => {
    const allBrands = bodyCareProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [bodyCareProducts, priceRange, minPrice, maxPrice, selectedBrands]);
  ...
  const productTypes = [
    { 
      label: "Dưỡng thể & massage", 
      imageUrl: "https://production-cdn.pharmacity.io/digital/256x256/plain/e-com/images/ecommerce/20240224170222-0-P10521_2.png",
      link: "/duong-the-massage"
    },
    { 
      label: "Tẩy tế bào chết toàn thân", 
      imageUrl: "https://production-cdn.pharmacity.io/digital/256x256/plain/e-com/images/product/20241113033306-0-P21165_1.png?versionId=xyabVICBBjsTJ.xEf7DoJfXtzvgJqSnW",
      link: "/tay-te-bao-chet-toan-than"
    },
  ];
      ...
      <main className="mx-auto max-w-screen-2xl px-6 py-8">
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Chăm sóc cơ thể</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Chăm sóc cơ thể</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
          ...
          <section>
            ...
            <BodyCareProductGrid products={filteredProducts} />
          </section>
        </div>
      </main>
