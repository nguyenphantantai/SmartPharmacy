...
export default function BloodGlucoseMetersPage() {
  ...
  const bloodGlucoseMetersProducts = useMemo(() => {
    return medicalDevicesProducts.filter(product => product.category === "Máy đo đường huyết");
  }, []);

  const brands = useMemo(() => {
    const allBrands = bloodGlucoseMetersProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [bloodGlucoseMetersProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = bloodGlucoseMetersProducts;
    ...
    return filtered;
  }, [bloodGlucoseMetersProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Máy đo đường huyết</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Máy đo đường huyết</h1>
