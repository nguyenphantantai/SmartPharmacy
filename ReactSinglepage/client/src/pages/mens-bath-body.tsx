...
export default function MensBathBodyPage() {
  ...
  const mensBathBodyProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Sản phẩm tắm & dưỡng thể cho nam");
  }, []);

  const brands = useMemo(() => {
    const allBrands = mensBathBodyProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [mensBathBodyProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = mensBathBodyProducts;
    ...
    return filtered;
  }, [mensBathBodyProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink href="/cham-soc-nam-gioi">Chăm sóc nam giới</BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sản phẩm tắm & dưỡng thể cho nam</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sản phẩm tắm & dưỡng thể cho nam</h1>
