...
export default function HairDyePage() {
  ...
  const hairDyeProducts = useMemo(() => {
    return personalCareProducts.filter(product => product.subCategory === "Nhuộm tóc");
  }, []);

  const brands = useMemo(() => {
    const allBrands = hairDyeProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [hairDyeProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink href="/cham-soc-toc">Chăm sóc tóc</BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Nhuộm tóc</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Nhuộm tóc</h1>
