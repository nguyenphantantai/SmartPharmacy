...
export default function MakeupRemoverPage() {
  ...
  const makeupRemoverProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Nước tẩy trang");
  }, []);

  const brands = useMemo(() => {
    const allBrands = makeupRemoverProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [makeupRemoverProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = makeupRemoverProducts;
    ...
    return filtered;
  }, [makeupRemoverProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Nước tẩy trang</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Nước tẩy trang</h1>
