...
export default function FaceBodySunscreenPage() {
  ...
  const faceBodySunscreenProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Kem chống nắng cho mặt và cơ thể");
  }, []);

  const brands = useMemo(() => {
    const allBrands = faceBodySunscreenProducts.map((p) => p.brand);
    return Array.from(new Set(allBrands));
  }, [faceBodySunscreenProducts]);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = faceBodySunscreenProducts;
    ...
    return filtered;
  }, [faceBodySunscreenProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink href="/san-pham-chong-nang">Sản phẩm chống nắng</BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Kem chống nắng cho mặt và cơ thể</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Kem chống nắng cho mặt và cơ thể</h1>
