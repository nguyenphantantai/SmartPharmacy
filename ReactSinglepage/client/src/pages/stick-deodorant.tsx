...
export default function StickDeodorantPage() {
  ...
  // Lọc sản phẩm sáp khử mùi
  const stickProducts = useMemo(() => {
    return personalCareProducts.filter(product => 
      product.category === "Sản phẩm khử mùi" && product.subCategory === "Sáp khử mùi"
    );
  }, []);

  const brands = useMemo(() => {
    const allBrands = stickProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [stickProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sáp khử mùi</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sáp khử mùi</h1>
