...
export default function AntiAgingProductsPage() {
  ...
  const antiAgingProducts = useMemo(() => {
    return beautyCareProducts.filter(product => product.subCategory === "Sản phẩm hỗ trợ chống lão hóa");
  }, []);

  const brands = useMemo(() => {
    const allBrands = antiAgingProducts.map((p) => p.brand);
  ...
  const filteredProducts = useMemo(() => {
    let filtered = antiAgingProducts;
    ...
    return filtered;
  }, [antiAgingProducts, priceRange, minPrice, maxPrice, selectedBrands]);
        ...
        <div className="mb-4">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sản phẩm hỗ trợ chống lão hóa</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

        <h1 className="text-2xl font-bold mb-5">Sản phẩm hỗ trợ chống lão hóa</h1>
